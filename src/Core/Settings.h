...
#define COMMON_SETTINGS(M, ALIAS) \
    ...
    M(UInt64, query_cache_max_size, std::numeric_limits<UInt64>::max(), "The maximum amount of memory (in bytes) the current user may allocate in the query cache.", 0) \
    M(UInt64, query_cache_max_entries, std::numeric_limits<UInt64>::max(), "The maximum number of query results the current user may store in the query cache.", 0) \
